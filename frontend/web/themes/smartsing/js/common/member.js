...
function setRealClock()
{
    if ($real_clock.length) {
        ...
        $real_clock.html(`${data.day}/${data.month}/${data.year}, ${data.hour}:${data.min}:${data.sec} ${$real_clock.data('timezone-short-name')}`);
...
function recalcLeftToNextLesson() {
    ...
    if ($minutes_left_to_next_lesson.length) {
        ...
        if (seconds_left < 60) {
            $('#minutes-left-text-before').html('меньше чем через');
        ...
        if (seconds_left > 0) {
            ...
            if ($schedule_info_block.length) {
                ...
                if (test_lesson_skipped <= -200) {
                    ...
                    $lesson_in_progress.html('Занятие пропущено');
...
$(document).ready(function() {
    ...
    /**/
    repairHeightLeftMenuWhenHidden();
