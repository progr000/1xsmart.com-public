...
(function (root, factory) {
    ...
    var Ssi_upload = function (element, options) {
        ...
        this.$element = '';
        this.language = (typeof locale[this.options.locale] != 'undefined') ? locale[this.options.locale] : locale['en'];
        this.uploadList = [];
        this.totalProgress = [];
        this.toUpload = [];
        this.imgNames = [];
    ...
    Ssi_upload.prototype.init = function (element) {
        ...
        var $chooseBtn = $('' +
            ...
            '<button id="ssi-chooseBtn" class="ssi-button success">' + this.language.chooseFiles + '</button>' +
            '</span>').append(element);
        ...
        if (thisS.options.dropZone) {
            $uploadBox.on("drop", function (e) {
                if (!thisS.options.multiple && thisS.currentListLength > 0) {
                    return false
                }
                e.preventDefault();
    ...
    Ssi_upload.prototype.toUploadFiles = function (files) {
        ...
        for (var i = 0; i < filesLength; i++) {
            ...
            if ($.inArray(ext, this.options.allowed) === -1) { // if requested file not allowed
                ...
                $uploadBtn.prop("disabled", false).removeClass('ssi-hidden');
                setupReader(file);
                    ...
                    fileReader.onload = function () {
                        ...
                        if (toUploadLength === j) {// if all elements are in place lets load images
                            ...
                            setTimeout(function () {
                                ...
                                $uploadBtn.prop("disabled", false).removeClass('ssi-hidden');
                                $clearBtn.prop("disabled", false).removeClass('ssi-hidden');
                            }, 10);
                            $uploadBtn.prop("disabled", false).removeClass('ssi-hidden');
                            $clearBtn.prop("disabled", false).removeClass('ssi-hidden');
    ...
    Ssi_upload.prototype.uploadFiles = function () {// upload the pending files
        ...
        function ajaxLoopRequest(formData, ii) {
            ...
            $.ajax(ajaxOptions).done(function (responseData, textStatus, jqXHR) {
                ...
                if (thisS.options.responseValidationFunction) {
                    try {
                        thisS.options.responseValidationFunction(data);
                    } catch (err) {
                        if (!thisS.options.ignoreCallbackErrors) {
    ...
    var locale = {
        en: {
            ...
            clear: 'Clear',
            drag: 'Drag n Drop',
            sizeError: '$1 Exceeded the maximum allowable file size  $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
        ...
        ru: {
            success: 'Успешно',
            sucUpload: 'Успешно загружено',
            chooseFiles: 'Выберите файл',
            uploadFailed: 'Загрузить файл',
            ...
            sizeError: '$1 Превышен максимально допустимый размер файла $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 формат файла не поддерживается',//$1=file extension ie(exe files are not supported)
            someErrorsOccurred: 'Произошли некоторые ошибки!',
            wentWrong: 'Что-то пошло не так!',
            pending: 'Приостановлено',
            completed: 'Выполнено',
            inProgress: 'Выполняется'
        },
        ua: {
            success: 'Успішно',
            sucUpload: 'Успішно завантажено',
            chooseFiles: 'Оберіть файл',
            uploadFailed: 'Завантажити файл',
            serverError: 'Внутрішня помиллка серверу',
            error: 'Помилка',
            abort: 'Перервати',
            aborted: 'Перервано',
            files: 'файли',
            upload: 'Завантажити',
            clear: 'Очистити',
            drag: 'Або перетащіть сюди',
            sizeError: '$1 Перевищено максимально допустимий розмір файлу $2',// $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
            extError: '$1 формат файлу не підтримується',//$1=file extension ie(exe files are not supported)
            someErrorsOccurred: 'Виникли деякі помилки!',
